                            ...
                            <tbody class="bg-white divide-y divide-gray-200 divide-solid">
                                @foreach($pengajuan as $p)
                                    <tr class="bg-white">
                                        ...
                                        <td>
                                            <form method="POST" action="{{ route('admin.pengajuan.approve', $p->id) }}">
                                                @csrf
                                                @method('PATCH')
                                                <button type="submit" class="inline-flex items-center rounded-md border border-green-300 bg-green-100 px-4 py-2 text-xs font-semibold uppercase tracking-widest text-green-700 shadow-sm transition duration-150 ease-in-out hover:bg-green-50 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 disabled:opacity-25">
                                                    Approve
                                                </button>
                                            </form>

                                            <form method="POST" action="{{ route('admin.pengajuan.reject', $p->id) }}">
                                                @csrf
                                                @method('PATCH')
                                                <button type="submit" class="inline-flex items-center rounded-md border border-red-300 bg-red-100 px-4 py-2 text-xs font-semibold uppercase tracking-widest text-red-700 shadow-sm transition duration-150 ease-in-out hover:bg-red-50 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 disabled:opacity-25">
                                                    Reject
                                                </button>
                                            </form>
                                        </td>
