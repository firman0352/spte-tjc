            ...
            <div class="overflow-hidden bg-white shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    ...
                    <div class="flex justify-between">
                        <div>
                            <p class="text-black"><strong class="text-gray-400">Address:</strong> {{ $verifikasi->dokumenCustomer->alamat_pt }}</p>
                        </div>
                    <div>
                    ...
                    @if(auth()->user()->inspektur->jabatan_id == 1)
                        @if($verifikasi->status_id == 6)
                            <div class="flex flex-row items-center mt-10 gap-4">
                                    <form method="post" action="{{ route('inspektur.verifikasi.approve', ['verifikasi' => $verifikasi]) }}" class="flex flex-col items-center">
                                        @csrf
                                        @method('patch')
                                        <button type="submit" class="hover:bg-green-600 text-white rounded-lg font-semibold bg-green-500 flex items-center">
                                            <svg fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" width="30px">
                                    ...
                                    <div class="flex flex-col items-center">
                                        <button onclick="openModal()" class="hover:bg-red-600 text-white rounded-lg font-semibold bg-red-500 flex items-center">
                                            <svg fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" width="30px">
                        ...
                        @if($verifikasi->status_id == 7)
                            <div class="flex flex-col items-center mt-10 gap-4">
                                    <form method="post" action="{{ route('inspektur.verifikasi.approve', ['verifikasi' => $verifikasi]) }}" class="flex flex-col items-center">
                                        @csrf
                                        @method('patch')
                                        <button type="submit" class="px-2 hover:bg-green-600 text-white rounded-full font-semibold bg-green-500 flex items-center">
                                            <svg fill="none" stroke="currentColor" stroke-width="3.5" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" width="20px">
                                                <path stroke-linecap="round" stroke-linejoin="round" d="M4.5 12.75l6 6 9-13.5"></path>
                                            </svg>
                                            <span class="text-white text-lg">Accept</span>
                                        </button>
                                    </form>
                                    <div class="flex flex-col items-center">
                                        <button onclick="openModal()" class="px-2 hover:bg-red-600 text-white rounded-full font-semibold bg-red-500 flex items-center">
                                            <svg fill="none" stroke="currentColor" stroke-width="3.5" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" width="20px">
                                                <path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12"></path>
                                            </svg>
                                            <span class="text-white text-lg">Decline</span>
                                        </button>
                                    </div>
                            </div>
                        @endif
                    ...
                    <div class="mt-1">
                        <a href="{{ route('inspektur.verifikasi.index') }}" class="font-bold text-indigo-600 hover:underline">Back</a>
                    </div>
                </div>
            </div>
        </div>
...
<!-- Modal -->
<div id="myModal" style="display: none; position: absolute; top: 0; left: 0; background: rgba(0, 0, 0, 0.5); width: 100%; height: 100%; z-index: 50;">
    <div style="background: white; width: 60%; margin: 10% auto; padding: 20px; border: 1px solid #888; box-shadow: 0 4px 8px 0 rgba(0,0,0,0.2);">
        ...
        <form method="post" action="{{ route('inspektur.verifikasi.reject', ['verifikasi' => $verifikasi]) }}">
            ...
            <button type="submit" class="inline-flex items-center rounded-md border border-red-300 bg-red-500 px-4 py-2 text-xs font-semibold uppercase tracking-widest text-white shadow-sm transition duration-150 ease-in-out hover:bg-red-600 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 disabled:opacity-25">Decline</button>
            <button type="button" onclick="closeModal()" class="inline-flex items-center rounded-md border border-gray-300 bg-white px-4 py-2 text-xs font-semibold uppercase tracking-widest text-gray-700 shadow-sm transition duration-150 ease-in-out hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 disabled:opacity-25">Cancel</button>
        </form>
    </div>
</div>
