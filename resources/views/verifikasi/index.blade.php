...
<x-app-layout>
    ...
    <x-verif-details-modal>
        ...
        <p class="mb-2 text-black"><strong class="text-gray-400">Inspector I :</strong> <span id="inspector1/address">Inspector I Name</span></p>
        <p class="mb-2 text-black"><strong class="text-gray-400">Inspector II :</strong> <span id="inspector2/phone">Inspector II Name</span></p>
        </x-slot>
    </x-verif-details-modal>
    ...
    <div>
        <div class="max-w-full px-4">
            <div class="flex justify-between">
                ...
                <x-filter-button href="{{ route('verifikasi.index', ['status_ids' => [6, 7]]) }}" :active="request()->has('status_ids') && (in_array(6, request()->input('status_ids')) || in_array(7, request()->input('status_ids')))">
                    Ongoing
                </x-filter-button>
                ...
                <x-filter-button href="{{ route('verifikasi.index', ['status_ids' => [4, 5]]) }}" :active="request()->has('status_ids') && (in_array(4, request()->input('status_ids')) || in_array(5, request()->input('status_ids')))">
                    Not successful
                </x-filter-button>

                </div>
                <a href="{{ route('verifikasi.index', ['status_ids' => []]) }}" class="text-indigo-500 font-bold">Reset Filter</a>
            </div>
                    ...
                    <div class="min-w-full align-middle">
                        <table id="listTable" class=" nowrap hover compact">
                            ...
                            <tr>
                                ...
                                <th class="bg-transparant px-6 py-3 text-left">
                                    <span class="text-xs font-bold uppercase leading-4 tracking-wider text-black">Inspector I</span>
                                </th>
                                <th class="bg-transparant px-6 py-3 text-left">
                                    <span class="text-xs font-bold uppercase leading-4 tracking-wider text-black">Inspector II</span>
                                </th>
                                <th class="bg-transparant px-6 py-3 text-left">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200 divide-solid">
                                @foreach($verifikasi as $item)
                                    <tr class="bg-white">
                                        ...
                                        <td class="px-6 py-4 text-sm leading-5 text-gray-900 whitespace-no-wrap dt-body-center w-1/5">
                                            <form action="{{ route('verifikasi.destroy', $item) }}" method="POST" data-id="{{ $item->id }}" style="display: inline-block;">
                                                @csrf
                                                @method('DELETE')
                                                <button type="button" class="delete-button inline-flex text-red-500 items-center px-2 py-2 hover:text-red-700 disabled:opacity-25">
